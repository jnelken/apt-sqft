...
export const useRoomManager = ({
  ...
  const handleAddRoom = useCallback((roomData: Omit<Room, 'id' | 'points'>) => {
  ...
  const handleUpdateRoom = useCallback((roomData: Omit<Room, 'id' | 'points'>) => {
  ...
  }, [appState.selectedRoomId, appState.floorPlan, pushToHistory, setAppState, setSidebarTab]);
